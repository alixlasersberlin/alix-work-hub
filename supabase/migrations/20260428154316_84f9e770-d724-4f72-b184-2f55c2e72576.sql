do $$
begin
  if exists (select 1 from cron.job where jobname = 'daily-order-sync') then
    perform cron.unschedule('daily-order-sync');
  end if;
end$$;
...
select cron.schedule(
  ...
  select net.http_post(
    url := 'https://xmrmkgfgpoundfwhnxfs.supabase.co/functions/v1/scheduled-order-sync',
