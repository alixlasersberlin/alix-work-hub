...
WITH CHECK (
  ...
  AND NOT (mfa_exempt IS DISTINCT FROM (SELECT up.mfa_exempt FROM public.user_profiles up WHERE up.id = auth.uid()))
  AND NOT (mfa_exempt_reason IS DISTINCT FROM (SELECT up.mfa_exempt_reason FROM public.user_profiles up WHERE up.id = auth.uid()))
  AND NOT (mfa_exempt_by IS DISTINCT FROM (SELECT up.mfa_exempt_by FROM public.user_profiles up WHERE up.id = auth.uid()))
  AND NOT (mfa_grace_until IS DISTINCT FROM (SELECT up.mfa_grace_until FROM public.user_profiles up WHERE up.id = auth.uid()))
);