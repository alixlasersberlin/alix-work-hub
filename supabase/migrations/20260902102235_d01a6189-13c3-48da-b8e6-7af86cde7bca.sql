...
BEGIN
  ...
  -- Existiert bereits eine echte Anzahlungsrechnung (AZ) zu diesem Auftrag?
  ...
  IF v_manual > 0 THEN
    -- Kein Auto-Datensatz zusätzlich: Duplikat vermeiden
...
-- Bestehende Duplikate entfernen
DELETE FROM public.finance_deposits a
 WHERE a.source_ref = 'order:' || a.order_id::text
   AND EXISTS (
     SELECT 1 FROM public.finance_deposits b
      WHERE b.order_id = a.order_id
        AND b.id <> a.id
        AND COALESCE(b.source_ref,'') <> 'order:' || a.order_id::text
   );