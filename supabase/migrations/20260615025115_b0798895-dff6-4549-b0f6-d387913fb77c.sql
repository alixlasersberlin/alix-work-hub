...
DECLARE
  tbls text[] := ARRAY[
    ...
    'zoho_invoices','zoho_recurring_invoices','zoho_recurring_profiles'
  ];
  t text;
  moved jsonb := '{}'::jsonb;
  rc integer;
...
BEGIN
  ...
  -- Re-point all customer_id references
  ...
  -- Fill missing fields on primary from duplicates
  UPDATE public.customers p SET
    ...
    street       = COALESCE(NULLIF(p.street,''),       d.street),
    zip          = COALESCE(NULLIF(p.zip,''),          d.zip),
    city         = COALESCE(NULLIF(p.city,''),         d.city),
    country      = COALESCE(NULLIF(p.country,''),      d.country),
  ...
  FROM (
    SELECT
      ...
      (array_agg(street)       FILTER (WHERE street       IS NOT NULL AND street<>''))[1]       AS street,
      (array_agg(zip)          FILTER (WHERE zip          IS NOT NULL AND zip<>''))[1]          AS zip,
      (array_agg(city)         FILTER (WHERE city         IS NOT NULL AND city<>''))[1]         AS city,
    ...
    FROM public.customers WHERE id = ANY(_duplicate_ids)
  ) d
