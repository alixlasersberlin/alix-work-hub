...
BEGIN
  ...
  -- Ensure mediapaket status is closed (skipped if nothing set)
  INSERT INTO public.as_mediapaket_status (case_id, stage, updated_by, updated_at)
  VALUES (_case_id, 'skipped'::as_media_stage, v_user, now())
  ON CONFLICT (case_id) DO UPDATE
    SET stage = CASE
                  WHEN public.as_mediapaket_status.stage IN ('done','skipped')
                    THEN public.as_mediapaket_status.stage
                  ELSE 'skipped'::as_media_stage
                END,
        updated_by = v_user,
        updated_at = now();
  ...
  UPDATE public.as_cases
     SET status = 'completed'::as_case_status,
         closed_at = now(),
         closed_by = v_user,
         progress_pct = 100,
         traffic_light = 'green'::as_traffic_light,
         updated_at = now()
   WHERE id = _case_id
