...
BEGIN
  ...
  SELECT count(*) INTO v_manual
   ...
   WHERE order_id = o.id
     AND COALESCE(source_ref, '') <> 'order:' || o.id::text;

  SELECT * INTO v_dep
    FROM public.finance_deposits
   WHERE order_id = o.id AND source_ref = 'order:' || o.id::text
   LIMIT 1;
  ...
  -- MwSt aus dem Auftrag (Standard 19 % in der EU, sonst 0)
  v_rate := COALESCE(o.az_tax_percentage,
                     CASE WHEN COALESCE(o.accounting_region::text, 'EU') = 'EU' THEN 19 ELSE 0 END);
  v_net := ROUND(v_soll / (1 + v_rate / 100.0), 2);
  v_vat := ROUND(v_soll - v_net, 2);

  IF v_paid >= v_soll THEN
    v_status  := 'bezahlt'; v_release := 'auto_freigegeben';
  ELSIF v_paid > 0 THEN
    v_status  := 'teilweise'; v_release := 'teilweise';
  ELSE
    v_status  := 'offen'; v_release := 'wartet';
  END IF;
...
-- Bestehende Auto-Datensätze ohne MwSt nachziehen (nur EU, nicht bezahlt/gebucht)
UPDATE public.finance_deposits d
   SET net_amount = ROUND(d.gross_amount / 1.19, 2),
       vat_amount = ROUND(d.gross_amount - ROUND(d.gross_amount / 1.19, 2), 2),
       updated_at = now()
  FROM public.orders o
 WHERE d.order_id = o.id
   AND d.source_ref = 'order:' || o.id::text
   AND COALESCE(d.accounting_region::text,'EU') = 'EU'
   AND COALESCE(o.az_tax_percentage, 19) = 19
   AND COALESCE(d.vat_amount, 0) = 0
   AND d.gross_amount > 0;