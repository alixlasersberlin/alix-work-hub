// Serves the public JS snippet: cookieless tracker + chat bubble.
const corsHeaders = {
...
const script = (apiKey: string) => `
(function(){
  if (window.__alixConnectLoaded) return; window.__alixConnectLoaded = true;
  var API_KEY = ${JSON.stringify(apiKey)};
  var TRACK = ${JSON.stringify(FN_BASE + '/ac-track')};
  var CHAT  = ${JSON.stringify(FN_BASE + '/ac-chat')};
  ...
  function sess(){var s=sessionStorage.getItem(SESS_KEY);if(!s){s=uuid();try{sessionStorage.setItem(SESS_KEY,s);}catch(e){}}return s;}
  ...
  function baseEvent(type){var utm=getUTM();return Object.assign({event_type:type,page_url:location.href,page_title:document.title,referrer:document.referrer,language:navigator.language,screen_size:screen.width+'x'+screen.height,session_hash:sess()},utm);}
  function send(events){try{navigator.sendBeacon(TRACK,new Blob([JSON.stringify({api_key:API_KEY,events:events})],{type:'application/json'}));}catch(e){fetch(TRACK,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({api_key:API_KEY,events:events}),keepalive:true}).catch(function(){});}}
  ...
  function mount(){
    ...
    function bubble(txt, mine){var b=h('div','max-width:80%;padding:8px 12px;border-radius:12px;line-height:1.4;white-space:pre-wrap;word-wrap:break-word;'+(mine?'align-self:flex-end;background:'+primary+';color:#fff;':'align-self:flex-start;background:#fff;color:#111;border:1px solid #e5e7eb;'),txt);body.appendChild(b);body.scrollTop=body.scrollHeight;}
    ...
    function renderStart(){
      ...
      form.onsubmit = function(ev){ev.preventDefault(); submit.disabled=true; submit.textContent='...';
...
Deno.serve((req) => {
  ...
  if (!apiKey) return new Response('// missing api key', { status: 400, headers: corsHeaders });
  return new Response(script(apiKey), { headers: corsHeaders });
});
