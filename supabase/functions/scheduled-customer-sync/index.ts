...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await adminClient.from("order_import_logs").insert({
      ...
      message: `Kunden (Cron) · ${totalFetched} geladen · ${totalImported} neu · ${totalUpdated} aktualisiert · ${totalSkipped} übersprungen · ${totalFailed} Fehler · ${Math.round(durationMs/1000)}s`,
    });
