// Alixsales.com inbound webhook → sales_leads
// Mirrors zoho-forms-import but with source="alixsales" and ALIXSALES_WEBHOOK_KEY.
...
const SHARED_KEY = Deno.env.get("ALIXSALES_WEBHOOK_KEY") ?? "";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const external_id =
      pick(data, [
        "id", "lead_id", "submission_id", "record_id",
        "form_id", "request_id", "uuid",
      ]) ?? crypto.randomUUID();
    const form_name = pick(data, [
      "form_name", "formName", "form", "source_form", "campaign", "kampagne",
    ]);
    ...
    const requested_products = pick(data, [
      "product", "products", "produktinteresse", "Produktinteresse",
      "interest", "geraet", "device",
    ]);
    const message = pick(data, [
      "message", "nachricht", "Nachricht", "comment", "kommentar", "notes",
    ]);
    ...
    const implementation_period = pick(data, [
      "implementation_period", "umsetzung", "umsetzungszeitraum",
      "Umsetzungszeitraum", "timeline",
    ]);
    let additional_services: any =
      data?.additional_services ?? data?.zusatzleistungen ?? data?.services ?? null;
    ...
    const service_rating = ratingRaw
      ? Math.max(1, Math.min(5, parseInt(ratingRaw, 10))) || null
      : null;
    ...
    if (uniqUsers.length > 0) {
      await supabase.from("mail_notifications").insert(
        uniqUsers.map((uid) => ({
          user_id: uid,
          type: "sales_lead",
          title: "Neue Anfrage von Alixsales.com",
          body: `${company ?? [fn, ln].filter(Boolean).join(" ") ?? "Neue Anfrage"} – ${requested_products ?? form_name ?? "Alixsales"}`,
