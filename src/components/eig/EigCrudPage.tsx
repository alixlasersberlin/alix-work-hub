...
import type { EigRecord } from "@/lib/eig/store";
...
interface Props { title: string; subtitle?: string; section: string; fields: CrudField[]; columns?: string[]; }

export default function EigCrudPage({ title, subtitle, section, fields, columns }: Props) {
  const [rows, setRows] = useState<EigRecord[]>(() => eig.list(section));
  ...
  const refresh = () => setRows(eig.list(section));

  const onExport = () => {
    const blob = new Blob([JSON.stringify(rows, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `${section}.json`; a.click(); URL.revokeObjectURL(url);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => { setDraft({}); setOpen(true); }}><Plus className="h-4 w-4 mr-1" />Neu</Button>
        </div>
      </div>

      <Card className="border-border/60 bg-card/40 backdrop-blur-xl">
        <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">{filtered.length} Einträge</CardTitle></CardHeader>
              ...
              {filtered.map(r => (
                <tr key={r.id} className="border-b border-border/30 hover:bg-accent/20">
                  ...
                  <td className="px-4 py-2 text-right whitespace-nowrap">
                    <Button size="sm" variant="ghost" onClick={() => { setDraft(r); setOpen(true); }}><Pencil className="h-3.5 w-3.5" /></Button>
                    <Button size="sm" variant="ghost" onClick={() => { eig.remove(section, r.id); refresh(); }}><Trash2 className="h-3.5 w-3.5 text-destructive" /></Button>
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && <tr><td colSpan={cols.length + 1} className="px-4 py-8 text-center text-muted-foreground text-sm">Keine Einträge</td></tr>}
            </tbody>
          </table>
        </CardContent>
      </Card>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { eig.upsert(section, draft); setOpen(false); setDraft({}); refresh(); }}>Speichern</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
