    ...
    async function load() {
      try {
        ...
        const [
          ...
          head(supabase.from('user_profiles')),
          head(supabase.from('user_profiles').eq('is_active', true)),
          head(supabase.from('login_sessions').eq('is_active', true).gt('expires_at', new Date().toISOString())),
          head(supabase.from('customers')),
          head(supabase.from('customers').eq('source_system', 'zoho_eu_1')),
          head(supabase.from('customers').eq('source_system', 'zoho_eu_2')),
          head(supabase.from('orders')),
          head(supabase.from('orders').eq('source_system', 'zoho_eu_1')),
          head(supabase.from('orders').eq('source_system', 'zoho_eu_2')),
          head(supabase.from('orders').in('order_status', ['offen', 'Offen', 'open', 'Open', 'approved', 'Approved', 'invoiced', 'Invoiced'])),
          head(supabase.from('orders').not('expected_shipment_date', 'is', null).lt('expected_shipment_date', today).not('order_status', 'in', '("geliefert","storniert","cancelled")')),
          ...
          head(supabase.from('production_orders')),
          head(supabase.from('production_orders').eq('approval_status', 'pending')),
          head(supabase.from('production_orders').eq('is_reclamation', true)),
          head(supabase.from('finance_records').eq('payment_status', 'offen')),
          ...
          head(supabase.from('tickets').not('status', 'in', '("closed","geschlossen","erledigt")')),
          head(supabase.from('repair_orders').not('repair_status', 'in', '("Abgeschlossen","abgeschlossen","Ausgeliefert","ausgeliefert","Storniert","storniert")')),
          head(supabase.from('route_plans')),
          head(supabase.from('route_plans').gte('planned_date', today).lt('planned_date', tomorrow)),
          head(supabase.from('lager_devices')),
          head(supabase.from('zoho_items')),
          supabase.from('zoho_items').select('stock_on_hand'),
          head(supabase.from('bugs').not('status', 'in', '("closed","geschlossen")')),
          head(supabase.from('capas').not('status', 'in', '("closed","geschlossen")')),
          head(supabase.from('warranty_records').eq('warranty_status', 'Aktiv')),
          head(supabase.from('audit_logs').gte('created_at', since24)),
          head(supabase.from('audit_logs').gte('created_at', since24).or('action.ilike.%fail%,action.ilike.%denied%,action.ilike.%unauthorized%,action.ilike.%block%,action.ilike.%suspicious%')),
