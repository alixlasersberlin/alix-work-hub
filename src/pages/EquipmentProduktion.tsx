  ...
  useEffect(() => {
    (async () => {
      ...
      const prodDevices = (data as LagerDevice[]).filter(
        (d) => getStatusFromNotes(d.notes) === 'Produktion',
      );
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-4">
      <PageHeader
        ...
        title="Produktion"
        subtitle={'Lagergeräte mit Status „Produktion"'}
      />
      ...
      <div className="rounded-lg border border-border bg-card">
        {loading ? (
          ...
          <div className="p-8 text-center text-muted-foreground">
            Keine Geräte aktuell in Produktion.
          </div>
        ) : (
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {filtered.map((d) => (
                <TableRow key={d.id}>
                  ...
                  <TableCell>
                    <Badge className="bg-amber-500/15 text-amber-500 border-amber-500/40">
                      Produktion
                    </Badge>
                  </TableCell>
                  <TableCell>
