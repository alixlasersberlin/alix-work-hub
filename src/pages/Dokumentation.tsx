...
import {
  ...
  Workflow, HelpCircle, ArrowLeft, ShieldCheck, Cpu,
} from 'lucide-react';
...
const workflowDetails: Record<string, WorkflowDetail> = {
  ...
  'Tourenplanung': {
    workflow: [
      'Aufträge mit Status „lieferbereit" und vorhandenem Lagergerät erscheinen automatisch in /tourenplanung.',
      'Planer legt unter /tourenplanung/neu eine Tour an: Datum, Fahrer, Fahrzeug, Region.',
      'Per Drag-and-Drop werden Aufträge der Tour zugewiesen — die Reihenfolge wird gespeichert.',
      'Edge Function calculate-driving-time berechnet via Google Maps Distance Matrix die Fahrzeit zwischen jeder Position.',
      'useDrivingTimes cached die Ergebnisse pro Tour, um API-Kosten zu sparen.',
      'Nach der Auslieferung markiert der Fahrer Stationen als „geliefert" → orders.status="geliefert" + lager_devices.area="Ausgeliefert".',
      'Einstellungen (/tourenplanung/einstellungen) verwalten Fahrer-Stammdaten, Fahrzeuge und Region-Cluster.',
    ],
    events: [
      'Änderung der Reihenfolge in einer Tour triggert calculate-driving-time neu (nur für betroffene Segmente).',
      'Wechsel auf „geliefert" stößt den Versand-Workflow an (PDF-Lieferschein + Customer-Shipping-Notice-Mail).',
      'Wird ein Auftrag aus einer Tour entfernt, kehrt er automatisch in die Prio-Liste zurück.',
      'Bei Erreichen des Liefertermins ohne Tour-Zuweisung wird der Auftrag im Dashboard als „überfällig" markiert.',
      'Touren-Historie bleibt nach Abschluss read-only erhalten (audit_locked=true).',
    ],
    guidelines: [
      ...
      'Touren-Historie bleibt zur Nachverfolgung erhalten — kein Hard-Delete.',
      'Google-Maps-API-Calls werden auf 1.000/Tag begrenzt; Cache wird priorisiert.',
    ],
  },
  'Versand': {
    workflow: [
      'Nach „geliefert"-Statuswechsel erzeugt das System automatisch einen Lieferschein-PDF (lib/pdf-utils.ts).',
      'send-customer-shipping-notice sendet die PDF per E-Mail an den Kunden (via send-transactional-email Edge Function mit Resend).',
      'Ratenplan-Aufträge (payment_type="installment") triggern automatisch die Ratenplan-Erzeugung mit Fälligkeiten.',
      'Mietkauf-Verträge werden separat unter /versand/mietkauf hochgeladen und mit dem Auftrag verknüpft.',
      'SEPA-Mandat: Kunde erhält Link, signiert digital (SepaMandatButton-Flow); Mandatsreferenz wird generiert und gespeichert.',
      'Übersicht /papiere zeigt alle Dokumente eines Auftrags an einem Ort.',
    ],
    events: [
      'E-Mail-Bounces werden über handle-email-suppression Edge Function verarbeitet — Adresse wird gesperrt.',
      'handle-email-unsubscribe entfernt Kunden aus Marketing-Listen (Unsubscribe-Link in jeder Mail).',
      'Fehlgeschlagene SEPA-Signaturen lösen eine erneute Aufforderung nach 48h aus.',
      'Mietkauf-Vertrag fehlt am Liefertag → Warnung im Dashboard und Blockade der „geliefert"-Quittung.',
      'Lieferscheine werden mit qualifizierter PDF-Signatur versehen (rechtssicher).',
    ],
    guidelines: [
      'Versandpapiere sind nach Erstellung unveränderlich; Korrekturen erfolgen über Gutschriften/Stornos.',
      'SEPA-Mandate folgen den geltenden Banken-Vorgaben (Gläubiger-ID, Mandatsreferenz nach SEPA-Format).',
      'Mietkauf-Verträge MÜSSEN vor Lieferung im System hinterlegt sein.',
      'E-Mail-Versand nur über send-transactional-email (keine direkten SMTP-Aufrufe aus dem Frontend).',
    ],
  },
  'Buchhaltung': {
    workflow: [
      'sync-zoho-invoices läuft stündlich und synchronisiert alle Rechnungen aus Zoho Books in die Tabelle invoices.',
      'sync-zoho-unpaid-invoices aktualisiert offene Posten alle 15 Minuten (separater Endpunkt für Performance).',
      'sync-zoho-recurring-invoices verarbeitet wiederkehrende Rechnungen täglich.',
      'Ratenzahler (/finance/ratenzahler) zeigt Plan-vs-Ist pro Kunde mit eingegangenen Zahlungen.',
      'Offene Posten (/finance/offene-posten) listen unbezahlte Rechnungen nach Fälligkeit, gruppiert in 0–30 / 31–60 / 61–90 / >90 Tagen.',
      'Mahnstufen 1–3 werden basierend auf Tagen Überfälligkeit automatisch berechnet und im UI angezeigt.',
    ],
    events: [
      'Bei Zahlungseingang in Zoho wird beim nächsten Sync invoices.status="paid" gesetzt; Auftrag verschwindet automatisch aus Offene Posten.',
      'Erreicht eine Rechnung Mahnstufe 3, erscheint sie als Vorschlag in der Anwaltsliste.',
      'Ratenzahler mit ausstehender Rate >14 Tage werden im Dashboard als „kritisch" markiert.',
      'Stornierte Rechnungen in Zoho werden lokal als invoices.voided=true markiert — keine Mahnung mehr.',
      'Wechselt eine Rechnung zwischen den Zahlungsstati, wird ein audit_log-Eintrag erzeugt.',
    ],
    guidelines: [
      'Finanzdaten sind read-only aus Sicht der App (Quelle: Zoho Books).',
      'Manuelle Zahlungseingänge werden in Zoho gebucht, nicht in der App.',
      'Zugriff nur für Rollen Finance, Admin und Super Admin.',
      'Mahnungen werden manuell ausgelöst, basierend auf den berechneten Stufen — kein Auto-Versand.',
    ],
  },
  'Finanzierungen': {
    workflow: [
      'Auftragsverwaltung markiert Auftrag als finanzierungsfähig → erscheint unter /finanzierungen/leasing-bank.',
      'Finanzierungs-Sachbearbeiter wählt Auftrag, klickt „Beantragen" → Formular mit Bank, Betrag, Laufzeit.',
      'Beim Absenden wird ein Eintrag in bank_financing_requests mit status="open" angelegt.',
      'Edge Function send-transactional-email sendet die Anfrage per Mail an die Bank (Template bank-financing-request).',
      'Rückmeldung der Bank wird manuell erfasst: Zusage → status="approved" oder Absage → status="rejected" mit Pflicht-Begründung.',
      'Bei Zusage wird die Finanzierungsreferenz im Auftrag verknüpft (orders.financing_id).',
    ],
    events: [
      'Anfrage bleibt nach 14 Tagen ohne Rückmeldung im Status „open" — Warnung im Dashboard für die Rolle Finanzierungen.',
      'Zusage triggert Aktualisierung des Auftrags-Status auf „finanziert" und entsperrt die Bestellung.',
      'Absage erzeugt eine Aufgabe für Auftragsverwaltung (alternative Finanzierung suchen oder Auftrag stoppen).',
      'Rolle „Finanzierungen" wird beim Login direkt auf /finanzierungen umgeleitet (HomeRoute-Logik in App.tsx).',
    ],
    guidelines: [
      'Bank-Korrespondenz wird zentral in bank_financing_requests protokolliert (inkl. Anhänge in Storage).',
      'Rolle „Finanzierungen" sieht ausschließlich diesen Bereich und Read-Only-Zugriff auf zugehörige Aufträge/Kunden.',
      'Absagen MÜSSEN mit Grund (min. 20 Zeichen) dokumentiert werden.',
      'Keine direkte Bank-API-Anbindung — Kommunikation erfolgt per E-Mail.',
    ],
  },
  'Operations': {
    workflow: [
      'Benutzerverwaltung (/benutzer): Admin lädt neuen User via send-user-invitation Edge Function ein → User erhält Mail mit /passwort-setzen-Link.',
      'Rollen werden in user_roles separat gespeichert (NIE auf user_profiles) — Prüfung erfolgt über has_role() (SECURITY DEFINER).',
      'Import (/import) startet manuelle Zoho-Syncs via start-zoho-import (Aufträge, Kunden, Items, Rechnungen).',
      'Monitoring (/system) zeigt letzte Ausführung, Dauer und Fehler aller scheduled-* Edge Functions.',
      'Logfiles (/operation/logfiles) listen Auth-Events, Rollenänderungen, Genehmigungen und kritische Operations.',
      'Datensicherung: nightly-backup erstellt täglich um 02:00 ein vollständiges DB-Backup; sync-backup-to-hetzner spiegelt es nach Hetzner Storage Box.',
      'E-Mail-Vorlagen (/operation/email-vorlagen) werden im Browser editiert und mit preview-transactional-email live gerendert.',
    ],
    events: [
      'create-user erzwingt MFA-Setup beim ersten Login (mfa_required=true im user_profiles-Eintrag).',
      'Rollenentzug entfernt den Eintrag aus user_roles → Sidebar-Menüs verschwinden beim nächsten Refresh sofort.',
      'admin-reset-password generiert temporäres Passwort und zwingt zum Wechsel beim nächsten Login.',
      'mfa-admin-reset löscht alle MFA-Faktoren des Users — er muss neu enrollen.',
      'Failed nightly-backup → Alert-Mail an Admin via backup-failure-alert Template.',
      'weekly-sync läuft jeden Sonntag und konsolidiert alle Zoho-Daten (Vollabgleich statt Inkrement).',
      'download-backup-zip erzeugt einen signierten Download-Link mit 1h-Gültigkeit.',
    ],
    guidelines: [
      'Benutzer-Anlage geschieht ausschließlich über Einladung (kein Self-Signup, kein öffentliches Registrierungsformular).',
      'Rollen-Änderungen sind audit-pflichtig — jeder Wechsel landet in den Logfiles.',
      'E-Mail-Vorlagen werden zentral gepflegt und versioniert (Git-History in supabase/functions/_shared/transactional-email-templates).',
      'Datensicherung muss monatlich auf Wiederherstellbarkeit getestet werden.',
      'Roles werden NIEMALS auf der user_profiles-Tabelle gespeichert — Risiko der Privilege-Escalation.',
      'Service-Role-Key NUR in Edge Functions — niemals im Frontend.',
    ],
  },
  'Authentifizierung & Sicherheit': {
    workflow: [
      'Nutzer öffnet /login → Cloudflare Turnstile prüft per verify-turnstile Edge Function, dass es kein Bot ist.',
      'Supabase Auth prüft E-Mail/Passwort; bei Erfolg wird eine Session erstellt.',
      'useAuth lädt user_profiles und alle Rollen aus user_roles (eine Tabelle, NIE auf Profil gespeichert).',
      'Wenn mfa_required=true und kein TOTP-Faktor existiert → Weiterleitung auf /mfa-setup (MfaGate).',
      'Bei vorhandenem Faktor → /mfa-challenge: 6-stelliger Code per TOTP-App oder Mail-OTP (send-otp-challenge + verify-otp-challenge).',
      'Recovery-Codes werden bei Setup mit mfa-store-recovery-codes verschlüsselt abgelegt; Einlösung via mfa-use-recovery-code.',
      'Eingeladene Nutzer erhalten Mail mit /passwort-setzen-Link (Token via Supabase Auth Reset-Flow).',
      'ReauthDialog erzwingt eine erneute Passwort-Eingabe vor sensiblen Aktionen (Rollenänderung, Bestellungs-Freigabe).',
    ],
    events: [
      'block_reason im user_profiles → useAuth leitet sofort auf /account-blocked um, ohne Datenzugriff.',
      '5 fehlgeschlagene Login-Versuche in 10 min → Turnstile erzwingt zusätzliche Challenge.',
      'mfa-admin-reset durch Admin → User muss beim nächsten Login TOTP neu enrollen.',
      'Sitzungs-Ablauf nach 24 h Inaktivität → Auto-Logout, Redirect /login.',
      'Fehlende Rolle auf einer geschützten Route → AccessDenied-Seite (keine 404, damit klar wird, dass es die Route gibt).',
      'Lieferant-Only-Logik: Wenn ausschließlich Rolle "Lieferant" vorhanden, leitet HomeRoute direkt auf /production.',
      'Finanzierungen-Only-Logik: Ausschließlich Rolle "Finanzierungen" → HomeRoute leitet auf /finanzierungen.',
    ],
    guidelines: [
      'MFA ist Pflicht für alle internen Nutzer — kein Bypass.',
      'Rollen werden NIEMALS auf user_profiles gespeichert (Privilege-Escalation-Risiko).',
      'has_role() ist eine SECURITY DEFINER Funktion und die einzige autorisierte Quelle für RBAC-Prüfungen in RLS.',
      'Recovery-Codes sind one-shot — nach Einlösung werden sie gelöscht.',
      'Service-Role-Key nur in Edge Functions, niemals im Frontend.',
      'Passwort-Reset-Links sind 1 Stunde gültig.',
      'Alle sicherheitsrelevanten Ereignisse landen in /operation/logfiles.',
    ],
  },
  'System & Integrationen': {
    workflow: [
      'Edge Functions laufen auf Supabase: planmäßig (scheduled-*), manuell (start-zoho-import) oder als Hook (auth-email-hook, handle-email-suppression).',
      'scheduled-order-sync (alle 30 min) und scheduled-customer-sync (stündlich) halten Aufträge und Kunden mit Zoho im Abgleich.',
      'scheduled-recurring-invoice-sync (täglich) verarbeitet Abo-Rechnungen aus Zoho Books.',
      'sync-zoho-invoices, sync-zoho-items, sync-zoho-packages, sync-zoho-unpaid-invoices laufen in eigenen Takten.',
      'nightly-backup (02:00) erzeugt ein vollständiges DB-Backup; sync-backup-to-hetzner spiegelt nach Hetzner Storage Box.',
      'weekly-sync (Sonntag) macht einen Vollabgleich aller Zoho-Daten zur Drift-Korrektur.',
      'send-transactional-email (Resend) ist der einzige Mail-Versandkanal — alle Vorlagen liegen in supabase/functions/_shared/transactional-email-templates.',
      'calculate-driving-time ruft Google Maps Distance Matrix und cached pro Tour.',
      'verify-turnstile prüft Cloudflare-Token serverseitig.',
    ],
    events: [
      'Fehlgeschlagener nightly-backup → automatischer Alert per backup-failure-alert Template an Admin.',
      'E-Mail-Bounce → handle-email-suppression markiert Adresse als gesperrt; weitere Sendungen werden unterbunden.',
      'Unsubscribe-Klick → handle-email-unsubscribe entfernt aus Marketing-Listen.',
      'Zoho-Sync-Fehler werden im Monitoring (/system) mit Stack-Trace angezeigt; bei kritischen Fehlern Mail an Admin.',
      'Theme-Wechsel (useTheme): Auswahl wird in localStorage gespeichert und beim Mount sofort angewandt — kein Flash.',
      'Mehrsprachigkeit im Production-Portal: user_profiles.preferred_language (de/en/zh) steuert die Anzeige.',
      'WelcomeDialog/LeoWelcomeDialog erscheinen einmalig pro User (Flag in user_profiles).',
      'download-backup-zip erzeugt einen signierten Download-Link mit 1 h Gültigkeit.',
    ],
    guidelines: [
      'Alle externen Aufrufe (Zoho, Google Maps, Resend, Hetzner) gehen ausschließlich über Edge Functions — niemals direkt aus dem Frontend.',
      'Secrets (API-Keys, Service-Role) liegen ausschließlich in Supabase Function Secrets, NICHT in der Datenbank oder im Code.',
      'Jede Edge Function ist idempotent — wiederholte Ausführung darf keine Duplikate erzeugen.',
      'Sync-Jobs schreiben Start/Ende/Fehler in eine Log-Tabelle für /system-Monitoring.',
      'Cron-Pläne sind in supabase/config.toml definiert und versioniert.',
      'Backup-Wiederherstellung muss monatlich getestet werden.',
      'Bei Schema-Änderungen IMMER eine Migration — niemals manuell in der Datenbank ändern.',
    ],
  },
  'Hilfe': {
    workflow: [
      'Support-Anfragen werden per E-Mail an rde@alix-lasers.com gesendet.',
      'Aktuelle Antwortzeit wird dynamisch berechnet (heute + 6 Wochen) und auf der Hilfe-Karte angezeigt.',
      'Diese Dokumentation (/hilfe/dokumentation) gibt eine vollständige Übersicht über alle Module, Workflows und Richtlinien.',
      'FAQ-Bereich sammelt häufig gestellte Fragen (in Entwicklung).',
    ],
    events: [
      'Hilfe-Menüpunkt ist für ALLE Rollen sichtbar — auch Lieferanten und Finanzierungen-Nutzer.',
      'Klick auf die Support-Mail öffnet das Standard-Mailprogramm via mailto:-Link.',
      'Dokumentation ist statisch im Frontend hinterlegt — keine Datenbank-Abhängigkeit (auch bei Backend-Ausfall verfügbar).',
    ],
    guidelines: [
      'Sensible Daten (Passwörter, MFA-Codes, API-Tokens) niemals per E-Mail teilen.',
      'IT-Team reagiert innerhalb der vereinbarten Service-Zeiten (siehe angezeigte Antwortzeit).',
      'Für sicherheitskritische Vorfälle: direkter Anruf statt E-Mail.',
    ],
  },
};
