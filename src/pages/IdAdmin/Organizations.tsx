...
type Org = {
  id: string; name: string; org_type: string; customer_id: string | null;
  tenant_id: string | null; country: string | null; status: string; created_at: string;
};
...
export default function IdAdminOrganizations() {
  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await supabase.from('alix_organizations')
        .select('id, name, org_type, customer_id, tenant_id, country, status, created_at')
        .order('created_at', { ascending: false }).limit(500);
      if (!error) setRows((data ?? []) as Org[]);
      setLoading(false);
    })();
  }, []);
  return (
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-border/40">
                    ...
                    <td className="py-2 pr-4">{r.org_type}</td>
                    <td className="py-2 pr-4 text-xs text-muted-foreground">{r.customer_id ?? '—'}</td>
