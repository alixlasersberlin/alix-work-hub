...
const BULK_TEMPLATE_BY_STATUS: Record<string, 'customer_warehouse_prepared' | 'customer_in_production' | 'customer_in_transit' | 'customer_warehouse_received' | 'customer_delivered'> = {
  ...
  'Ausgeliefert': 'customer_delivered',
};
        ...
        {loading ? (
                  ...
                  <div className="flex flex-wrap justify-end gap-1 pt-1">
                    ...
                    {d.reserved_order_id && (
                      <Button
                        variant="outline"
                        size="sm"
                        className="gap-1 h-8 border-amber-500/50 text-amber-500 hover:bg-amber-500/10 hover:text-amber-400 font-semibold"
                        onClick={() => releaseReservation(d)}
                        title="Reservierung von Auftrag und Gerät aufheben"
                      >
                        <Link2Off className="w-4 h-4" /> FREIGEBEN
                      </Button>
                    )}
          ...
          <Table>
            ...
            <TableBody>
              {sortedDevices.map((d) => {
                const kunde = parseLeihKunde(d.notes);
                const shots = parseSchusszahl(d.notes);
                const leihStartVal = parseLeihStart(d.notes);
                    ...
                    <div className="flex justify-end gap-1">
                      ...
                      {d.reserved_order_id && (
                        <Button
                          variant="outline"
                          size="sm"
                          className="gap-1 border-amber-500/50 text-amber-500 hover:bg-amber-500/10 hover:text-amber-400 font-semibold"
                          onClick={() => releaseReservation(d)}
                          title="Reservierung von Auftrag und Gerät aufheben"
                        >
                          <Link2Off className="w-4 h-4" /> FREIGEBEN
