...
export default function FinanceBudget() {
  ...
  async function load() {
    setLoading(true);
    const { data: rows } = await supabase.from('finance_budgets' as any).select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).eq('fiscal_year', year);
    const map: BudgetMap = {};
  ...
  async function save() {
    ...
    for (const cat of Object.keys(data)) {
      for (let m = 1; m <= 12; m++) {
        rows.push({ fiscal_year: year, month: m, category: cat, planned_amount: data[cat]?.[m] || 0, accounting_region: (String(region) === 'ALL' ? 'EU' : region) });
      }
    }
