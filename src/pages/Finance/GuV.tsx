  ...
  useEffect(() => {
    (async () => {
      ...
      const [t, a, ii] = await Promise.all([
        supabase.from('finance_transactions').select('amount, transaction_type, booking_date').gte('booking_date', s).lte('booking_date', e).in('accounting_region', region === 'ALL' ? ['EU','CH'] : [region]),
