  ...
  useEffect(() => {
    (async () => {
      ...
      const [t, a, r] = await Promise.all([
        supabase.from('finance_transactions').select('amount, transaction_type, booking_date, customer:customer_id(source_system, company_name, contact_name)').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).gte('booking_date', yearStart).order('booking_date'),
        supabase.from('finance_accounts').select('customer_id, current_balance, overdue_balance, customers:customer_id(company_name, contact_name, source_system)').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('overdue_balance', { ascending: false }).limit(50),
