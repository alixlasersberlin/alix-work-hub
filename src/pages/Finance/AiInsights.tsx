...
export default function FinanceAiInsights() {
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from('finance_ai_insights' as any).select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('created_at', { ascending: false }).limit(50);
  ...
  async function generate() {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('finance-ai-analyze', {
        body: {
          scope, period_start: s, period_end: e, accounting_region: (String(region) === 'ALL' ? 'EU' : region),
