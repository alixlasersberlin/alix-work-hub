...
export default function FinanceAnomalien() {
  ...
  async function load() {
    setLoading(true);
    let q: any = supabase.from('finance_anomalies' as any).select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('detected_at', { ascending: false }).limit(500);
