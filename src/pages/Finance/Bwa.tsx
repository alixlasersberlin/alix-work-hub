  ...
  useEffect(() => {
    (async () => {
      ...
      const [t, tp, a, ii] = await Promise.all([
        supabase.from('finance_transactions').select('amount, transaction_type, booking_date').gte('booking_date', start).lte('booking_date', end).in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]),
        supabase.from('finance_transactions').select('amount, transaction_type, booking_date').gte('booking_date', startPrev).lte('booking_date', endPrev).in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]),
