  ...
  useEffect(() => {
    (async () => {
      ...
      const [t, a, ii, bl] = await Promise.all([
        supabase.from('finance_transactions').select('amount, transaction_type, booking_date').in('accounting_region', (String(region) === 'ALL' ? ['EU','CH'] : [region]) as any).gte('booking_date', s),
        supabase.from('finance_accounts').select('current_balance, overdue_balance, last_payment_at').in('accounting_region', (String(region) === 'ALL' ? ['EU','CH'] : [region]) as any),
        supabase.from('finance_incoming_invoices').select('amount_gross, invoice_date, due_date, paid_at').in('accounting_region', (String(region) === 'ALL' ? ['EU','CH'] : [region]) as any),
        supabase.from('finance_bank_lines').select('amount, value_date').in('accounting_region', (String(region) === 'ALL' ? ['EU','CH'] : [region]) as any),
      ]);
      setTx(t.data ?? []);
      setAccounts(a.data ?? []);
      setIncoming(ii.data ?? []);
