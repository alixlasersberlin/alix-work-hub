  ...
  return (
    <div className="p-4 lg:p-6 space-y-4">
      ...
      <Tabs defaultValue="coa" className="space-y-4">
        ...
        <TabsContent value="coa"><CoaTab region={region as any} canWrite={isAdmin} canDelete={isSuperAdmin} /></TabsContent>
        <TabsContent value="tax"><TaxTab region={region as any} canWrite={isAdmin} canDelete={isSuperAdmin} /></TabsContent>
        <TabsContent value="cc"><CcTab region={region as any} canWrite={isAdmin} canDelete={isSuperAdmin} /></TabsContent>
        <TabsContent value="cu"><CuTab region={region as any} canWrite={isAdmin} canDelete={isSuperAdmin} /></TabsContent>
        <TabsContent value="per"><PerTab region={region as any} canWrite={isAdmin} canDelete={isSuperAdmin} /></TabsContent>
        <TabsContent value="ob"><ObTab region={region as any} canWrite={isAdmin} canDelete={isSuperAdmin} /></TabsContent>
      </Tabs>
    </div>
  );
}
...
function CoaTab({ region, canWrite, canDelete }: { region: 'EU'|'CH'; canWrite: boolean; canDelete: boolean }) {
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('finance_chart_of_accounts').insert({
      ...form, accounting_region: (String(region) === 'ALL' ? 'EU' : region),
    } as any);
...
function CcTab({ region, canWrite, canDelete }: { region: 'EU'|'CH'; canWrite: boolean; canDelete: boolean }) {
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('finance_cost_centers').insert({ ...form, accounting_region: (String(region) === 'ALL' ? 'EU' : region) } as any);
    if (error) return toast.error(error.message);
...
function CuTab({ region, canWrite, canDelete }: { region: 'EU'|'CH'; canWrite: boolean; canDelete: boolean }) {
  ...
  const save = async () => {
    if (!form.code || !form.name) return toast.error('Code & Name erforderlich');
    const { error } = await supabase.from('finance_cost_units').insert({ ...form, accounting_region: (String(region) === 'ALL' ? 'EU' : region) } as any);
    if (error) return toast.error(error.message);
...
function PerTab({ region, canWrite, canDelete }: { region: 'EU'|'CH'; canWrite: boolean; canDelete: boolean }) {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase.from('finance_periods')
      .select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).eq('fiscal_year', year)
      .order('period_month');
    if (error) toast.error(error.message); else setRows((data as any) || []);
    setLoading(false);
  };
  ...
  const ensureAll = async () => {
    ...
    for (let m=1; m<=12; m++) if (!existing.has(m)) toCreate.push({ accounting_region: (String(region) === 'ALL' ? 'EU' : region), fiscal_year: year, period_month: m, status: 'open' });
  ...
  const setStatus = async (r: PER|null, month: number, status: PER['status']) => {
    if (r) {
      ...
      const { error } = await supabase.from('finance_periods').insert({
        accounting_region: (String(region) === 'ALL' ? 'EU' : region), fiscal_year: year, period_month: month, status,
...
function ObTab({ region, canWrite, canDelete }: { region: 'EU'|'CH'; canWrite: boolean; canDelete: boolean }) {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase.from('finance_opening_balances')
      .select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).eq('fiscal_year', year).order('account_number');
    if (error) toast.error(error.message); else setRows((data as any) || []);
    setLoading(false);
  };
  useEffect(() => { load(); }, [region, year]);
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('finance_opening_balances').insert({ ...form, accounting_region: (String(region) === 'ALL' ? 'EU' : region), fiscal_year: year } as any);
    if (error) return toast.error(error.message);
...
function TaxTab({ region, canWrite, canDelete }: { region: 'EU'|'CH'; canWrite: boolean; canDelete: boolean }) {
  ...
  const load = async () => {
    ...
    const [{ data: c }, { data: w }] = await Promise.all([
      supabase.from('finance_tax_codes' as any).select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('code'),
      region === 'CH'
  ...
  const saveCode = async () => {
    ...
    const { error } = await supabase.from('finance_tax_codes' as any).insert({ ...codeForm, accounting_region: (String(region) === 'ALL' ? 'EU' : region) } as any);
    if (error) return toast.error(error.message);
