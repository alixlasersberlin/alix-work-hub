...
export default function Revisionsexport() {
  ...
  async function fetchRows(p: Pack) {
    let q: any = (supabase as any).from(p.table).select('*').eq('accounting_region', region).limit(10000);
  ...
  async function loadCounts() {
    ...
    for (const p of PACKS) {
      try {
        let q: any = (supabase as any).from(p.table).select('id', { count: 'exact', head: true }).eq('accounting_region', region);
