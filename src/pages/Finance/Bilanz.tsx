  ...
  useEffect(() => {
    (async () => {
      ...
      const [a, ac, bl, ii] = await Promise.all([
        ...
        supabase.from('finance_bank_lines').select('amount, value_date, statement_id').lte('value_date', stichtag).in('accounting_region', (String(region) === 'ALL' ? ['EU','CH'] : [region]) as any),
