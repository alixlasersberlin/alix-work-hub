...
export default function FinanceIntercompany() {
  ...
  const load = async () => {
    ...
    const [{ data: t }, { data: r }, { data: u }] = await Promise.all([
      ...
      supabase.from('finance_intercompany_relations' as any).select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('created_at', { ascending: false }),
      supabase
  ...
  const addRelation = async () => {
    ...
    const { error } = await supabase.from('finance_intercompany_relations' as any).insert({
      source_tenant_id: source, target_tenant_id: target, label: label || null, accounting_region: (String(region) === 'ALL' ? 'EU' : region),
    });
