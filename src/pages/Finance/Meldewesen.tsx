...
export default function FinanceMeldewesen() {
  ...
  const load = async () => {
    ...
    const [{ data: t }, { data: f }] = await Promise.all([
      ...
      supabase.from('finance_tax_filings' as any).select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('created_at', { ascending: false }).limit(100),
    ]);
    setTenants((t ?? []) as any);
    setFilings((f ?? []) as any);
    setLoading(false);
  ...
  const generate = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('finance-tax-export', {
        body: { filing_type: tab, period_value: period, tenant_id: tenantId || null, accounting_region: (String(region) === 'ALL' ? 'EU' : region) },
      });
