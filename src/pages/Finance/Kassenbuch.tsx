...
export default function Kassenbuch() {
  ...
  async function load() {
    setLoading(true);
    let q: any = (supabase as any).from('finance_cashbook').select('*').gte('booking_date', from).lte('booking_date', to).in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('booking_date', { ascending: false }).order('booking_time', { ascending: false });
