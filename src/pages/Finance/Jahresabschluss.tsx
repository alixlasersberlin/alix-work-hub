...
export default function FinanceJahresabschluss() {
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from('finance_year_end_runs' as any).select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('fiscal_year', { ascending: false });
  ...
  async function createRun() {
    ...
    const { error } = await supabase.from('finance_year_end_runs' as any).insert({ fiscal_year: newYear, status: 'in_arbeit', checklist: {}, accounting_region: (String(region) === 'ALL' ? 'EU' : region) });
