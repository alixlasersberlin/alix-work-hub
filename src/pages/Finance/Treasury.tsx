...
export default function FinanceTreasury() {
  ...
  const load = async () => {
    ...
    const [{ data: t }, { data: a }, { data: l }, { data: p }] = await Promise.all([
      ...
      supabase.from('finance_bank_accounts' as any).select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('created_at', { ascending: false }),
      supabase.from('finance_liquidity_entries' as any).select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('entry_date', { ascending: false }).limit(60),
      supabase.from('finance_payment_approvals' as any).select('*').in('accounting_region', String(region) === 'ALL' ? ['EU','CH'] : [region]).order('created_at', { ascending: false }).limit(80),
    ]);
    setTenants((t ?? []) as any);
