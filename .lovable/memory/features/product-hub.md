...
- **Migrationsphase in `ph_settings.migration_phase`: seit 16.08.2026 Phase C — Canary (BlueIce Smart KI) inkl. Website-Rendering verifiziert, COM→DE Legacy-Sync ist abgeschaltet (`com_de_sync_active=false`), AlixWork ist alleiniges Master-System (`alixwork_master=true`). Quell-Endpoint für Importe: https://alix-legacy-reborn.lovable.app/api/public/product-hub/export (Secret DE_EXPORT_API_KEY).**
