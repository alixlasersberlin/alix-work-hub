...
- Stufe 3 (Teil): `Artikel.tsx` lädt die Liste ohne `raw_data` (explizite Spaltenliste `LIST_COLUMNS`); Rohdaten werden per `openDetail()` erst beim Öffnen nachgeladen. `Customers.tsx` nutzt bereits die RPC `customers_page` + `customers_letters` (serverseitige Pagination), `Invoices.tsx` lädt `raw_data` ebenfalls nur bei Bedarf.
- Such-Indizes (trgm) ergänzt: orders(external_order_id, internal_number, case_number), zoho_invoices(customer_name, reference_number), lager_devices(serial_number, model_name), zoho_items(description). Doppelte Kunden-Indizes `idx_customers_company_trgm`/`idx_customers_contact_trgm` gelöscht.
- Messwerte: Artikelsuche über Name/SKU/Beschreibung ~6 ms, Reklamations-Zähler ~2 ms.
- Lagergeräte: Auftragssuche (freie Aufträge + Modell-Vorschläge) läuft serverseitig über `order_items`/`customers` statt 500 Aufträge inkl. Positionen je Tastendruck; neue Trigram-Indizes auf `order_items(item_name, description, sku)` + `orders(order_status, customer_id)`. Messung: Positionssuche ~0,7 ms.
